...
5. Configure PHP's local mail transport (the MTA used by PHP `mail()`) for outbound delivery. Quote notifications are sent to `info@printadvertisings.com`; an optional plain `PA_MAIL_FROM` address can be supplied in the PHP hosting environment, but no mail password or API key belongs in the site. Mail failures are logged without returning mail or database details to the browser, and the saved lead remains available to the dashboard.
